...
with col1:
    ...
    with tab_about:
        ...
        st.write("**Harmonee** is built using Python and Streamlit. It uses the *abracadabra* audio-recognizing-library as a base for audio fingerprinting and several other libraries like *pydub, pyaudio* or *lyricsgenius* for additional functionality. The database is managed using *TinyDB*, The album covers displayed on recognition are integrated through *DuckDuckGo-search*. Lyrics are loaded from *Genius.com*.")
        st.write("The source code for abracadabra is available on GitHub at https://github.com/notexactlyawe/abracadabra")
        st.write("Harmonee is developed by Andreas Thöni, Samuel Peer and Hannes Unterhuber as part of the final project for the course *Introduction to Softwaredesign* at the MCI Innsbruck.")
        st.write("The absolute mess of a source code for **Harmonee** is available on GitHub at https://github.com/SUhonzz/Harmonee")
...
with col2:
    ...
    if artist is not None and title is not None:
        ...
        with st.container(border=True):
            cover, info = st.columns([0.5, 0.5])
            with cover:
                st.image('./design/no_album_static.png', caption='No Matched Song', use_column_width=True, output_format='PNG')
            with info:
                with st.container(border=True):
                    st.subheader('Artist')
